...
module Dijkstra
  def shorest_path_query(src_id, dst_id, attr_postions)
    dijkstra_init(src_id, attr_postions)
  ...
  def dijkstra_init(src_id, attr_postions)
    @attr_postions = attr_postions
  ...
  def length_between(src, dst)
    @edges_hash[[src, dst]].attrs[@attr_postions]
  end
end
